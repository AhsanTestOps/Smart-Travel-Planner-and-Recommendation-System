...
const FreeMapDisplayEnhanced = () => {
  ...
  useEffect(() => {
    if (location.state?.freeTrip) {
      ...
      geocodeDestination(location.state.freeTrip.destination);
    }
  }, [location.state]);
  ...
  const getNearbyAttractions = async (lat, lon, destination) => {
    ...
    const interests = selectedTrip?.interests ? selectedTrip.interests.split(',').map(i => i.trim().toLowerCase()) : [];
    ...
    interests.forEach((interest, index) => {
      if (attractionTypes[interest] && index < 3) {
        const options = attractionTypes[interest];
        ...
        mockAttractions.push({
          ...
          type: interest.replace(' & ', '_').replace(' ', '_'),
          ...
          <div className="mb-6">
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                {selectedTrip.budget && (
                  <p>💰 ${selectedTrip.budget} USD</p>
                )}
                <p>👥 {selectedTrip.adults} traveler{selectedTrip.adults > 1 ? 's' : ''}</p>
              </div>
            </div>
          </div>
          ...
          {selectedTrip.interests && (
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedTrip.interests.split(',').map((interest, index) => (
                  <span
                    key={index}
...
export default FreeMapDisplayEnhanced;
